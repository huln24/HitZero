...
public class RecursiveHitZero {
    ...
    private static boolean recursiveHitZero(int[] A, int i) {
        // Empty array of same size as game array, content at index i to be updated when we visit that position
    ...
    // Checks if we return to already visited position, so we know whether game is solvable or not
    private static boolean recursiveHitZero_(int[] A, int i, int[] B) {
        int size = A.length;
        printGameBoard(A);
        printMarker(A,i);
        // If we already visited the position i (B[i] = 1),
        // we know that game is unsolvable because we're returning to where we were before
        if (B[i] == 1) {
            return false;
        }
        ...
        return false;    // We're stuck at current position, because we can't move to the both directions by A[i]. Hence, game unsolvable
    }
    ...
    private static void gameTest() {
        ...
        for (int i = 0; i < game.length; i++) {
            ...
            else System.out.println("Game is not solvable!\n");
